...
# PUT /salary - Replace (set) salary for current month
@router.put("/salary")
async def set_salary(salary: Salary):
    ...
    if existing_salary:
        await salary_collection.update_one(
            {"month": current_month},
            {"$set": {"amount": salary.amount, "currency": salary.currency}},
        )
        return {
            "message": f"Salary set to ₹{salary.amount:,.0f} for {current_month}",
            "previous_salary": existing_salary["amount"],
            "total_salary": salary.amount,
            "currency": salary.currency,
        }
